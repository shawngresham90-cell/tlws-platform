...
begin
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Speed up freight movement, standardize industry paperwork, and lower carriers'' insurance costs"},{"key":"b","text":"License shippers, tax dangerous cargo, and track foreign freight"},{"key":"c","text":"Keep the product in its package, make the danger plain to everyone, and keep the drivers and rigs that move it safe"},{"key":"d","text":"Protect cargo value, prevent theft, and simplify customs inspections"}]'::jsonb,
   'c',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"They are listed at the bottom of the page, after all of the ordinary freight entries"},{"key":"b","text":"They are attached on a separate page, kept in a sealed envelope with the manifest"},{"key":"c","text":"They are entered first, highlighted in a contrasting color, or marked with an X in the HM column"},{"key":"d","text":"They are written in red ink and initialed by the driver at pickup"}]'::jsonb,
   'c',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"In a pouch on the driver''s door, or in clear view within your reach with the seat belt on"},{"key":"b","text":"Locked in the glove box, so they cannot be stolen while you sleep"},{"key":"c","text":"Taped inside the trailer, next to the cargo they describe, where the load is"},{"key":"d","text":"On file at the carrier''s main office, with a copy at the shipper''s dock"}]'::jsonb,
   'a',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"In your pocket, so the load is never separated from its papers"},{"key":"b","text":"Clipped to the sun visor on the passenger side, out of the weather"},{"key":"c","text":"In your trip envelope in the overhead console, with the rest of the paperwork"},{"key":"d","text":"On the driver''s seat or in the driver''s-door pouch"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"A printed route survey signed by the carrier''s safety officer before dispatch"},{"key":"b","text":"Emergency response information for the material, and a phone number monitored around the clock"},{"key":"c","text":"The shipper''s certificate of insurance, showing coverage for the load on board"},{"key":"d","text":"A copy of the driver''s hazmat training record from the past three years"}]'::jsonb,
   'b',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Carrier fleet codes assigned by the FMCSA when the carrier registers"},{"key":"b","text":"Serial numbers that track the package through the carrier''s terminals"},{"key":"c","text":"Identification numbers assigned to the material in the hazardous materials table"},{"key":"d","text":"Customs tariff codes for freight that crosses an international border"}]'::jsonb,
   'c',
   'Those are identification numbers from the hazardous materials table. UN numbers are international, NA numbers are North American, and the number pins down exactly which material is in the package — which is why it must match the shipping paper.',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Placard the vehicle, no matter how small the package"},{"key":"b","text":"Carry it in the cab, where you can watch it for leaks during the trip"},{"key":"c","text":"Placard the vehicle only past 1,001 pounds, the same as other poisons"},{"key":"d","text":"Double-wrap the package in plastic sheeting and load it near the doors"}]'::jsonb,
   'a',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"A working dome light and a mounted fire axe within the driver''s reach"},{"key":"b","text":"Steel flooring, washed down and still damp before the first package loads"},{"key":"c","text":"No sharp points that could damage cargo, and a tight floor liner"},{"key":"d","text":"At least one open vent on each side wall to keep vapors from collecting"}]'::jsonb,
   'c',
  ...
  (v_test,
   ...
   '49 CFR 177.842', '2026-07-17', 3, '{hazmat,radioactive}', 22),
  ...
  (v_test,
   'A "safe haven" is:',
   '[{"key":"a","text":"Any truck stop with 24-hour security cameras"},{"key":"b","text":"The shoulder of a divided highway, during daylight hours"},{"key":"c","text":"Any garage or yard the carrier owns"},{"key":"d","text":"A location approved in writing for parking unattended explosives"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"A high idle, to keep the air system charged while pumping"},{"key":"b","text":"Engine off, with a person in control of the nozzle the entire time"},{"key":"c","text":"Fueling only at the carrier''s own terminal pumps"},{"key":"d","text":"Grounding the vehicle to the pump island before the nozzle goes in"}]'::jsonb,
   'b',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Only during the pre-trip inspection, before the vehicle first moves"},{"key":"b","text":"Once per week, by a mechanic who signs the inspection report"},{"key":"c","text":"Every 500 miles, or at every fuel stop, whichever comes first"},{"key":"d","text":"At the start of the trip and each time the vehicle is parked"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Stop 15 to 50 feet before the nearest rail, then cross without shifting gears"},{"key":"b","text":"Slow to 10 mph, downshift, and roll across without stopping"},{"key":"c","text":"Stop only when the warning lights are flashing or a train is in sight"},{"key":"d","text":"Sound the horn, check the mirrors, and cross in your highest gear"}]'::jsonb,
   'a',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Any transit delay of more than two hours caused by the release"},{"key":"b","text":"Every release of any size, even a few drops caught by the drip pan"},{"key":"c","text":"A death, an injury requiring hospitalization, or an evacuation caused by the release"},{"key":"d","text":"Only releases involving radioactive material or explosives"}]'::jsonb,
   'c',
   'The phone call goes in at the earliest practical moment when a release kills or hospitalizes someone, forces an evacuation, closes a major road for an hour or more, or involves radioactive or infectious materials. A detailed written report (DOT Form 5800.1) follows within 30 days.',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Giving route details to anyone who says they are from your carrier''s dispatch"},{"key":"b","text":"Leaving the rig locked and idling at truck stops so it always looks occupied"},{"key":"c","text":"Discussing the cargo over the CB only with drivers whose voices you recognize"},{"key":"d","text":"Verifying who you deliver to, and keeping route and cargo details private"}]'::jsonb,
   'd',
