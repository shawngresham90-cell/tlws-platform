...
begin
  ...
  values
  (v_test,
   'Which three braking systems share the same hardware on an air-brake-equipped vehicle?',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Check the compressor''s oil level instead of the belt"},{"key":"b","text":"Check the belt''s condition and tightness"},{"key":"c","text":"Remove the belt to inspect the pulley"},{"key":"d","text":"Check the belt only at scheduled service intervals"}]'::jsonb,
   'b',
  ...
  (v_test,
   ...
   'b',
   'The governor is the compressor''s on/off switch: it stops the pumping at cut-out pressure and resumes it at cut-in. It manages tank pressure — it has nothing to do with road speed on an air-brake system.',
  ...
  (v_test,
   ...
   'c',
   'Typical numbers to memorize: cut-out around 125 psi, cut-in around 100 psi. If you see the needle keep climbing well past cut-out, the governor or compressor has a problem and needs attention before you drive.',
   'CDL Manual §5.1.2', '2026-07-16', 2, '{air-brakes,governor}', 4),

  (v_test,
   'Why must air tanks be drained regularly?',
   '[{"key":"a","text":"To keep the tanks from rusting shut"},{"key":"b","text":"Draining raises the system pressure"},{"key":"c","text":"To reset the governor"},{"key":"d","text":"Water and compressor oil collect inside and can freeze in cold weather, causing brake failure"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Nothing — that is what it is for"},{"key":"b","text":"Drain the tanks and keep driving"},{"key":"c","text":"Plug the valve so pressure can build"},{"key":"d","text":"Have the fault fixed by a mechanic — something is wrong with the system"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"S-cam drum brakes"},{"key":"b","text":"Wedge brakes"},{"key":"c","text":"Band brakes"},{"key":"d","text":"Air disc brakes"}]'::jsonb,
   'a',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Any movement at all"},{"key":"b","text":"About an inch or more of free play at the push-rod connection"},{"key":"c","text":"More than three inches"},{"key":"d","text":"Slack adjusters are not an inspection item"}]'::jsonb,
   'b',
  ...
  (v_test,
   ...
   'c',
   'When system pressure bleeds down into the 20–45 psi range, the springs overcome the remaining air and apply fully. That is why you must safely stop as soon as the low-air warning activates — waiting risks an abrupt spring-brake application wherever you happen to be.',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"The service brakes being in adjustment"},{"key":"b","text":"How much weight rests on the drive axles"},{"key":"c","text":"The air compressor running"},{"key":"d","text":"The tractor protection valve"}]'::jsonb,
   'a',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"It drains the fuel tank"},{"key":"b","text":"It burns out the stop-light switch"},{"key":"c","text":"It releases the parking brakes"},{"key":"d","text":"The combined spring and air forces could damage the brakes"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Flashes the headlights"},{"key":"b","text":"Swings a mechanical arm down into the driver''s line of sight when pressure gets too low"},{"key":"c","text":"Vibrates the steering wheel"},{"key":"d","text":"Locks the throttle"}]'::jsonb,
   'b',
   'The wig wag''s arm drops when pressure falls below roughly 60 psi and stays in your view until pressure is restored — a warning you physically cannot miss or ignore.',
  ...
  (v_test,
   'Engine off, service brakes released: what air-loss rate is acceptable for a single vehicle?',
   '[{"key":"a","text":"Less than 2 psi per minute"},{"key":"b","text":"Less than 6 psi per minute"},{"key":"c","text":"Less than 10 psi per minute"},{"key":"d","text":"Up to 15 psi per minute"}]'::jsonb,
   'a',
  ...
  (v_test,
   'With the brake pedal held fully applied (engine off), what leakage rate is acceptable for a single vehicle?',
   '[{"key":"a","text":"Less than 3 psi per minute"},{"key":"b","text":"Less than 8 psi per minute"},{"key":"c","text":"No drop at all is acceptable"},{"key":"d","text":"Less than 12 psi per minute"}]'::jsonb,
   'a',
   'The applied leakage check: hold the pedal down for a minute after the initial drop and watch the gauge. A single vehicle must lose less than 3 psi per minute (less than 4 for a combination); anything worse is a leak in the applied side — chambers, lines, or valves — that must be repaired first.',
   'CDL Manual §5.2', '2026-07-16', 3, '{air-brakes,leakage-tests,inspection}', 19),

  (v_test,
   'What is the main advantage of a dual air brake system?',
   '[{"key":"a","text":"It doubles your stopping power"},{"key":"b","text":"It lets you drive with a known air leak"},{"key":"c","text":"It removes the need for spring brakes"},{"key":"d","text":"If one system loses air, the other can still stop the vehicle"}]'::jsonb,
   'd',
   'Two separate systems share one set of controls: typically one covers the front brakes and the other the rear. A failure in one leaves the other working — enough to stop safely, not to continue the trip. Stop and get it repaired.',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"You have no brakes — stop immediately on the shoulder"},{"key":"b","text":"Only the trailer brakes work"},{"key":"c","text":"The spring brakes are about to apply"},{"key":"d","text":"You still have normal brakes; the anti-lock function may not be working"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Pump the pedal rapidly to help the computer"},{"key":"b","text":"Press twice as hard as normal on every stop"},{"key":"c","text":"Use only the parking brake knob"},{"key":"d","text":"Brake as you always would — ABS only takes over when wheels are about to lock"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Air brakes are weaker than hydraulic brakes"},{"key":"b","text":"Air needs time to reach the brake chambers before they apply"},{"key":"c","text":"The driver must pump the pedal first"},{"key":"d","text":"They do not add any distance"}]'::jsonb,
   'b',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Perception distance + reaction distance + brake lag distance + effective braking distance"},{"key":"b","text":"Perception distance + reaction distance + shifting distance + braking distance"},{"key":"c","text":"Reaction distance + brake lag distance + braking distance + downgrade distance"},{"key":"d","text":"Perception distance + reaction distance + following distance + braking distance"}]'::jsonb,
   'a',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Cold drums after a rest stop"},{"key":"b","text":"Driving with the ABS lamp on"},{"key":"c","text":"Too much air pressure in the tanks"},{"key":"d","text":"Excessive heat from using the brakes too much instead of the engine''s braking effect"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Stores the vehicle''s reserve air supply"},{"key":"b","text":"Warns the driver of low pressure"},{"key":"c","text":"Dries the air before it reaches the tanks"},{"key":"d","text":"Converts air pressure into the push-rod force that applies the brake"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Engine off, key on, fan the brake pedal to bleed pressure down until the warning activates"},{"key":"b","text":"Rev the engine to maximum rpm and listen"},{"key":"c","text":"Disconnect the air lines at the tank"},{"key":"d","text":"Watch the warning light while idling at high rpm"}]'::jsonb,
   'a',
   'With enough air in the system, shut the engine off, turn the key to on, and fan the brake pedal to bleed pressure down. The light or buzzer must come on before pressure falls below 60 psi. If it never activates, you would get no warning of a real air loss — do not drive until it works.',
