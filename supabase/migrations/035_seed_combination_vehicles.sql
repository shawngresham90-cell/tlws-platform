...
begin
  ...
  (v_test,
   ...
   '[{"key":"a","text":"It is about the same"},{"key":"b","text":"It is lower — the weight plants the trailer"},{"key":"c","text":"It is roughly ten times higher"},{"key":"d","text":"It is about twice as high"}]'::jsonb,
   'c',
   'Load raises the center of gravity, and a high center of gravity is what tips trailers. Crash data puts the rollover rate for loaded rigs at roughly ten times the empty rate — the load does not plant you, it towers over you.',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Keep the cargo low and take turns slowly"},{"key":"b","text":"Use the trailer hand valve and brake hard in curves"},{"key":"c","text":"Load the cargo high and centered"},{"key":"d","text":"Grease the fifth wheel and rotate tires on schedule"}]'::jsonb,
   'a',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Quick steering wears the kingpin and fifth wheel out of round"},{"key":"b","text":"The \"crack-the-whip\" effect amplifies sudden moves at the trailer, which can tip it over"},{"key":"c","text":"Sharp inputs make the front tires scrub and overheat on long trips"},{"key":"d","text":"Smooth steering keeps the trailer brakes from dragging in curves"}]'::jsonb,
   'b',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"The bobtail stops much shorter — less weight"},{"key":"b","text":"They stop in the same distance"},{"key":"c","text":"The bobtail stops shorter, but only on wet pavement"},{"key":"d","text":"The bobtail can take longer to stop than the loaded rig"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Swing wide to the left before you start the turn"},{"key":"b","text":"Turn from the center lane at higher speed"},{"key":"c","text":"Finish the turn wide while your trailer tires hug the curb line"},{"key":"d","text":"Stop in the intersection and reverse first"}]'::jsonb,
   'c',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Cross without stopping when no stop is required"},{"key":"b","text":"Use a gear low enough to cross without accelerating"},{"key":"c","text":"Shift gears"},{"key":"d","text":"Straddle the rails with your drive axles"}]'::jsonb,
   'c',
  ...
  (v_test,
   ...
   'c',
   'The service line is the messenger for normal braking: it carries air only when you apply the brakes, telling the trailer how hard to brake. The other line — the supply line — is the one that constantly charges the trailer''s tanks.',
  ...
  (v_test,
   ...
   'a',
   'Loss of emergency-line pressure applies the trailer''s emergency brakes automatically — that is the fail-safe that stops a breakaway trailer before it can travel far on its own.',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"100 to 125 psi"},{"key":"b","text":"60 to 80 psi"},{"key":"c","text":"45 to 60 psi"},{"key":"d","text":"20 to 45 psi"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   'b',
   'The red eight-sided knob controls the trailer''s air supply: pushed in, it charges the trailer''s tanks. The yellow diamond is the tractor parking brake — a different control for a different job.',
   'CDL Manual §6.2', '2026-07-17', 1, '{combination,air-lines,controls}', 15),

  (v_test,
   'Why is it dangerous to use the trailer hand valve (trolley valve) while driving?',
   '[{"key":"a","text":"It overrides the tractor brakes completely"},{"key":"b","text":"It can lock the trailer wheels and start a jackknife"},{"key":"c","text":"It burns out the service line diaphragm"},{"key":"d","text":"It disables the trailer ABS for the rest of the trip"}]'::jsonb,
   'b',
   'The hand valve brakes the trailer alone, and braking only the trailer at road speed is how trailer wheels lock and swing out of line. Leave it stowed in traffic — the foot brake works both ends of the rig together.',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Air can leak away and let the brakes release"},{"key":"b","text":"The hand valve applies only the tractor brakes, not the trailer''s"},{"key":"c","text":"It sets the trailer ABS fault lamp"},{"key":"d","text":"It wears the slack adjusters out of alignment"}]'::jsonb,
   'a',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"That both couplers spin freely under load"},{"key":"b","text":"That the couplers are the same color as the lines"},{"key":"c","text":"That grease covers the seals before locking"},{"key":"d","text":"That the seals are clean and sound, and each line meets its matching coupler"}]'::jsonb,
   'd',
  ...
  (v_test,
   'Where is the ABS malfunction lamp on a trailer built since March 1998?',
   '[{"key":"a","text":"On the left side of the trailer, where you can see it in your mirror"},{"key":"b","text":"Inside the cab only"},{"key":"c","text":"Centered on the trailer nose"},{"key":"d","text":"On the right side, beside the marker lights"}]'::jsonb,
   'a',
   'Air-braked trailers built since March 1998 must have ABS, and the yellow malfunction lamp is mounted on the trailer''s left side — front or rear corner — where your mirror can catch it. Lamp on means the anti-lock function may be out; normal trailer brakes still work.',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Yanking the release arm by hand to feel for play"},{"key":"b","text":"Backing hard against the locked trailer brakes"},{"key":"c","text":"Rocking the trailer with the landing gear half raised"},{"key":"d","text":"Pulling gently forward against the locked trailer brakes"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"The trailer nose touches the back of the cab"},{"key":"b","text":"The release arm points straight up"},{"key":"c","text":"You can slide a hand between the coupler plates"},{"key":"d","text":"The coupler plates sit flush and the jaws grip the kingpin''s shank"}]'::jsonb,
   'd',
   'Get under with a light and look: daylight between the coupler plates means the trailer is riding on the jaws, and jaws closed around the kingpin''s wider head instead of its narrow shank will let go under load.',
   'CDL Manual §6.4', '2026-07-17', 3, '{combination,coupling,inspection}', 24),

  (v_test,
   'Before driving away coupled, the landing gear must be:',
   '[{"key":"a","text":"Touching the ground for backup support"},{"key":"b","text":"Fully raised, with the crank handle secured"},{"key":"c","text":"Raised just enough to clear level pavement"},{"key":"d","text":"Lowered slightly as a safety catch in case the coupling fails"}]'::jsonb,
   'b',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Lower it anyway — the trailer weight will firm the soil"},{"key":"b","text":"Uncouple with the trailer brakes released so the trailer can settle"},{"key":"c","text":"Place solid boards or supports under the landing gear feet"},{"key":"d","text":"Lower the gear only partway so the feet cannot sink"}]'::jsonb,
   'c',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"The fifth wheel plate is coated in a layer of grease"},{"key":"b","text":"The release arm sits flush against its stop"},{"key":"c","text":"The slider is pinned in its rearmost position"},{"key":"d","text":"The safety latch is not engaged over the release arm"}]'::jsonb,
   'd',
  ...
  (v_test,
   ...
   '[{"key":"a","text":"Drive at 30 mph and brake hard once"},{"key":"b","text":"Charge the system, pull out the trailer air supply valve, and tug gently against it"},{"key":"c","text":"Fan the brake pedal until the low-air warning sounds"},{"key":"d","text":"Watch the dashboard gauges for one minute with the brakes released"}]'::jsonb,
   'b',
   'Charge the trailer''s air, pull the trailer air supply valve to apply the trailer emergency brakes, then ease against them in low gear. If the rig creeps, the emergency system that is supposed to stop a runaway trailer is not doing its job.',
  ...
  (v_test,
   ...
   'c',
   'With normal pressure, brakes released, roll forward at a crawl and apply the hand valve alone. Feeling the trailer drag the rig down proves its service brakes answer the service line''s signal.',
