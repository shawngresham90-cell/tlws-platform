/**
 * Batch 14 (I-40 Tennessee) assessment — read-only, drives the REAL production libs:
 *  prepareImport, assessExpansion, classifyPair, scoreCompleteness, normalizeText.
 *
 *   npx esbuild scratch-b14/assess-b14.ts --bundle --platform=node --format=cjs \
 *     --alias:@=./src --outfile=.next/assess-b14.cjs && node .next/assess-b14.cjs \
 *     data/imports/i40-tennessee-batch-014.csv scratch-b14/live.json scratch-b14
 */
...
if (!masterPath || !livePath || !outDir) { console.error('usage: assess-b14 <master.csv> <live.json> <out-dir>'); process.exit(2); }
...
writeFileSync(`${outDir}/../data/imports/i40-tennessee-batch-014-expansion-report.csv`, expansionReportCsv(report));
...
const newPairs = csvToPairs(masterPath, 'b14');
...
const ky65Hits = crossCheck('PENDING I-65 Kentucky b010', csvToPairs('scratch-b14/pending-i65-kentucky-batch-010.csv', 'ky65p'));
const tn24Hits = crossCheck('PENDING I-24 Tennessee b011', csvToPairs('scratch-b14/pending-i24-tennessee-batch-011.csv', 'tn24p'));
const ky24Hits = crossCheck('PENDING I-24 Kentucky b012', csvToPairs('scratch-b14/pending-i24-kentucky-batch-012.csv', 'ky24p'));
const ilga24Hits = crossCheck('PENDING I-24 IL+GA b013', csvToPairs('scratch-b14/pending-i24-illinois-georgia-batch-013.csv', 'ilga24p'));
...
writeFileSync(`${outDir}/dup-summary.json`, JSON.stringify({ gaHits, tnHits, kyHits, ohHits, miHits, flHits, inHits, alHits, tn65Hits, ky65Hits, tn24Hits, ky24Hits, ilga24Hits, liveHits, inFile, inBatchSlugDupes: inBatchSlugDupes.length }, null, 1));
console.log('DONE');
