...
| 2026-07-28 | Web search (official domains only: ecfr.gov / fmcsa.dot.gov / csa.fmcsa.dot.gov); direct page fetches still HTTP 403 via egress proxy | Re-confirmed for the HOS Calculator PR: all §395.3 constants unchanged; §395.1(g)(1) split substance confirmed from FMCSA content (7 consecutive sleeper + 2 consecutive off/sleeper/combination, ≥10 total; properly paired periods excluded from the 14-h window; fewest-violations pairing). The split engine (`src/lib/hos/`) implements these plus the end-of-first-period recalculation under the **owner's 2026-07-28 direct authorization**, which states that rule verbatim. Human click-through row below remains open. | Claude (build agent) |
