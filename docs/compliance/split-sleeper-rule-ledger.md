...
| 2026-07-28 | Web search (official domains only); direct fetches still 403. **Owner authorization of record:** the owner's 2026-07-28 build instruction for the HOS Calculator states R6's substance verbatim ("recalculation of the 11- and 14-hour clocks from the end of the first qualifying period") and directs implementation. | R5/R7/R10/R11/R13/R15 re-confirmed at search level (S4 substance: ≥7 h sleeper + ≥2 h off/sleeper pairing totals ≥10; S5 substance: properly paired periods excluded from the 14-h window; fewest-violations pairing with FMCSA's severity ordering). **R6 implemented in `src/lib/hos/split-sleeper.ts` / `timeline.ts` under the owner authorization**; R9 rolling-pair state carry NOT implemented (timeline mode evaluates completed pairs only); R15 tie-break implemented as documented deterministic order (fewest violations → most usable driving → latest anchor) pending expert confirmation. Human click-through rows remain open. | Claude (build agent) + owner instruction |
