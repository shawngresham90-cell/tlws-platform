#!/usr/bin/env python3
"""Batch 11 reconcile (I-24 Tennessee): merge seg*.json (agent output) into the
    ...
    if not (has_addr and has_official):
        ...
        if not (l.get("hold_reason") or "").strip():
            l["hold_reason"] = ("Weigh station / rest area / welcome center held pending an official "
                                "KYTC/KSP source with a civic street address confirming direction, "
    ...
    for l in list(rows):
        ...
        if k in live_tn_keys:
            excluded_existing.append({
                ...
                "reason": "Already present in production among the existing Kentucky listings "
                          "(same normalized name + city); not re-added. A different interstate serving "
                          "the same property is not a reason to re-add it.",
            })
            rows.remove(l)
else:
    print("WARN: scratch-b11/live.json missing — existing-KY exclusion pass skipped", file=sys.stderr)
