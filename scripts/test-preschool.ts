...
check('price is 199', PRESCHOOL_PRICE_USD === 199);
check('price label is $199', PRESCHOOL_PRICE_LABEL === '$199');
...
check(
  'checkout disclosure says no card collection',
  /never collect card/i.test(CHECKOUT_DISCLOSURE),
);
...
check(
  'EMPTY_WALL zero state',
  EMPTY_WALL.filled === 0 && EMPTY_WALL.remaining === 20 && EMPTY_WALL.students.length === 0,
);
...
const named = {
  spotNumber: 2,
  displayName: 'Big Mike R.',
  isAnonymous: false,
  businessName: 'Mike Trucking',
  websiteUrl: 'https://miketrucking.example',
};
...
check(
  'blank name falls back safely',
  publicDisplayName({ ...named, displayName: '  ' }) === 'Founding Student',
);
...
check(
  'wall sorts numbered spots first, in order',
  sorted[0].spotNumber === 1 && sorted[1].spotNumber === 3 && sorted[2].spotNumber === null,
);
...
check(
  ...
  !claimSchema.safeParse({ ...validClaim, consent_public_display: false }).success,
);
check(
  'unconfirmed checkout rejected',
  !claimSchema.safeParse({ ...validClaim, confirmed_checkout: false }).success,
);
check(
  'bad email rejected',
  !claimSchema.safeParse({ ...validClaim, purchaser_email: 'not-an-email' }).success,
);
check(
  'http website rejected',
  !claimSchema.safeParse({ ...validClaim, website_url: 'http://example.com' }).success,
);
check(
  'short display name rejected',
  !claimSchema.safeParse({ ...validClaim, display_name: 'A' }).success,
);
check(
  'empty optional website ok',
  claimSchema.safeParse({ ...validClaim, website_url: '' }).success,
);
...
check('offer price 199.00', offer.price === '199.00');
...
check(
  'availability is the real LimitedAvailability',
...
check(
  'canonical ends with /cdl-pre-school',
  String(meta.alternates?.canonical ?? '').endsWith('/cdl-pre-school'),
);
...
check(
  'purchase CTA event name',
  PRESCHOOL_EVENTS.purchaseCtaClick === 'preschool_purchase_cta_click',
);
check('curriculum event name', PRESCHOOL_EVENTS.curriculumExpand === 'preschool_curriculum_expand');
check(
  'claim started event name',
  PRESCHOOL_EVENTS.claimStarted === 'founding_student_claim_started',
);
check(
  'claim submitted event name',
  PRESCHOOL_EVENTS.claimSubmitted === 'founding_student_claim_submitted',
);
const claimFormSrc = src('src/components/preschool/ClaimForm.tsx');
check(
  'claim events carry no props (no personal data)',
  /trackEvent\(PRESCHOOL_EVENTS\.claimStarted\)/.test(claimFormSrc) &&
    /trackEvent\(PRESCHOOL_EVENTS\.claimSubmitted\)/.test(claimFormSrc),
);
...
check(
  'module lesson counts sum to 33',
  allModules.reduce((n, m) => n + m.lessons, 0) === LESSON_COUNT,
);
check(
  'module numbers are 1..7 in order',
  allModules.map((m) => m.number).join(',') === '1,2,3,4,5,6,7',
);
for (const title of [
  ...
  check(
    `verified module title: ${title}`,
    allModules.some((m) => m.title === title),
  );
}
...
for (const heading of [
  ...
  check(
    `curriculum group heading: ${heading}`,
    CURRICULUM_GROUPS.some((g) => g.heading === heading),
  );
}
check(
  'whats-included mentions workbooks',
  WHATS_INCLUDED.some((b) => /workbook/i.test(b)),
);
check(
  'whats-included states the real quiz gate',
  WHATS_INCLUDED.some((b) => b.includes('80%')),
);
...
check(
  'access FAQ says enrollment is personal, not automated',
  /personally enrolls|issued by hand/.test(accessFaq?.answer ?? ''),
);
check('access FAQ mentions welcome email login', /welcome email/.test(accessFaq?.answer ?? ''));
const publicCopy =
  JSON.stringify(FAQS) + JSON.stringify(WHATS_INCLUDED) + JSON.stringify(CURRICULUM_GROUPS);
...
check(
  'no refund FAQ / no invented refund policy',
  !FAQS.some((f) => /refund/i.test(f.question + f.answer)),
);
check('sales page makes no refund claim', !/refund/i.test(salesSrcFull()));
check(
  'no success/employment guarantee wording',
  !/guarantee[ds]? (a |your )?(cdl|job|employment|pass)/i.test(publicCopy),
);
...
check(
  'sales page uses price constant, not a stray price',
  !/\$1[0-9]9/.test(salesSrc.replace(/PRESCHOOL_PRICE_LABEL/g, '')),
);
...
check(
  'wall empty state copy present',
  wallSrc.includes('Founding Student spots are now available'),
);
...
check(
  'every admin action gated by requireAdmin',
  actionCount > 0 && gateCount === actionCount,
  `${gateCount}/${actionCount}`,
);
...
check(
  'no fake urgency on homepage card',
  !/timer|countdown|only today|recently purchased/i.test(fourSrc),
);
...
check(
  'migration locks claims table (RLS, no anon)',
  migrationSrc.includes('enable row level security') &&
    migrationSrc.includes('revoke all on public.preschool_founding_claims'),
);
...
check(
  'migration blocks duplicate pending emails',
  migrationSrc.includes('preschool_claims_pending_email_uidx'),
);
...
check(
  'meter is pure (no data fetching)',
  !meterSrc.includes('supabase') && !meterSrc.includes("from '@/lib/preschool/data'"),
);
...
check(
  'four paths still no fake urgency',
