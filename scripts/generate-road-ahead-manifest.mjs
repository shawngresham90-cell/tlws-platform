  ...
  try {
    ...
    for (const [slot, value] of Object.entries(raw)) {
      ...
      if (id) youtube[slot] = id;
        ...
        console.warn(
          `[road-ahead] youtube-sources: could not parse a video id for "${slot}" (value: ${JSON.stringify(value)})`,
        );
    }
  } catch (e) {
...
const youtubeSorted = Object.fromEntries(
  Object.entries(youtube).sort(([a], [b]) => a.localeCompare(b)),
);
...
if (existsSync(footageFile)) {
  try {
    ...
    for (const m of list) {
      ...
      if (!norm) {
        console.warn(
          `[road-ahead] footage.json: skipped an unusable moment ${JSON.stringify(m)?.slice(0, 80)}`,
        );
        continue;
      }
      if (moments[norm.slot]) {
        console.warn(
          `[road-ahead] footage.json: slot "${norm.slot}" already has a moment — keeping the first, ignoring a later one.`,
        );
        continue;
      }
...
const momentsSorted = Object.fromEntries(
  Object.entries(moments).sort(([a], [b]) => a.localeCompare(b)),
);
