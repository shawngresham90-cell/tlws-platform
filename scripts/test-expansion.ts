...
import {
  assessExpansion,
  expansionReportCsv,
  expansionTemplateCsv,
} from '@/lib/directory/expansion';
...

{
  ...
  check(
    'rows: prepared rows exclude parser duplicates',
  ...
  check(
    'slug: live collision gets suffix',
    newStop.slugPreview === 'new-stop-dalton-ga-2' && newStop.slugCollision,
  );
  ...
  check(
    'verdict: invalid coords force manual review',
    badCoords.verdict === 'manual-review' && badCoords.geocoding === 'invalid-coords',
  );

  const thin = byName.get('Thin Stop')!;
  check(
    'verdict: thin row imports unpublished',
    thin.verdict === 'import-unpublished',
    thin.verdict,
  );
  check('geocoding: missing coords flagged', thin.geocoding === 'needs-geocoding');

  check(
    'counts: verdict buckets add up',
    report.verdictCounts['ready-to-publish'] +
      report.verdictCounts['import-unpublished'] +
      report.verdictCounts['manual-review'] ===
      report.rows.length,
  );
   ...
   classifier must flag it for manual review. */
{
  const csv = toCsv([
    HEADER,
    row('Pilot Travel Center Calhoun', { city: 'Calhoun', address: '324 Carbondale Rd' }),
  ]);
  const report = assessExpansion(csv, EXISTING_KEYS, EXISTING_SLUGS, LIVE);
  check(
    'duplicates: near-identical name vs live → manual review',
    report.rows[0]?.verdict === 'manual-review',
    report.rows[0],
  );
  check(
    'duplicates: hit names the live listing',
    report.rows[0]?.duplicateHits.some((h) => h.liveName.includes('Pilot')),
  );
}
...
/* formula guard + template */
{
  ...
  check(
    'template: single header row with name+category',
    template.length === 1 &&
      template[0].includes('Business Name') &&
      template[0].includes('Category'),
  );
}
