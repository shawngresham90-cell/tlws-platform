...
const POOL: DirectoryEntry[] = [
  ...
  entry({
    ...
    lng: -84.9152,
    interstate: 'I-75',
    amenities: ['Fuel', 'Free parking'],
  }),
  entry({
    id: 'scale4561',
    name: 'CAT Scale Valdosta',
    category: 'cat-scales',
    city: 'Valdosta',
    lat: 30.7543,
    lng: -83.2732,
...
/* ------------------------- filters ------------------------- */
{
  ...
  check(
    'filters: category',
    applyExploreFilters(POOL, { ...EMPTY_FILTERS, category: 'cat-scales' }, null)
      .map((e) => e.id)
      .join() === 'scale4561',
  );
  check(
    'filters: state',
    applyExploreFilters(POOL, { ...EMPTY_FILTERS, state: 'tn' }, null)
      .map((e) => e.id)
      .join() === 'ta-knox',
  );
  check(
    'filters: interstate',
    applyExploreFilters(POOL, { ...EMPTY_FILTERS, interstate: 'I-40' }, null).length === 1,
  );
  check(
    'filters: city',
    applyExploreFilters(POOL, { ...EMPTY_FILTERS, city: 'calhoun' }, null).length === 1,
  );
  check(
    'filters: single amenity',
    applyExploreFilters(POOL, { ...EMPTY_FILTERS, amenities: ['Showers'] }, null).length === 2,
  );
  check(
    'filters: AND amenities',
    applyExploreFilters(POOL, { ...EMPTY_FILTERS, amenities: ['Showers', 'Repair'] }, null)
      .map((e) => e.id)
      .join() === 'ta-knox',
  );
  check(
  ...
  check(
    ...
    !hasActiveFilters(EMPTY_FILTERS) && hasActiveFilters({ ...EMPTY_FILTERS, city: 'x' }),
  );
}
...
/* ------------------------- radius + nearest-first ------------------------- */
{
  ...
  check(
    'origin: distances attached',
    near.every((e) => e.distanceMiles != null),
  );
  check(
    'origin: nearest first',
    near[0].id === 'pilot67' && near[1].id === 'loves735',
    near.map((e) => `${e.id}:${e.distanceMiles}`),
  );
...
/* ------------------------- visibility invariants ------------------------- */
{
  ...
  check(
    'markers: only coordinate rows',
    markers.length === 4 && !markers.some((m) => m.id === 'nocoords' || m.id === 'halfcoords'),
  );
...
/* ------------------------- manual search (query parsing) ------------------------- */
{
  ...
  check(
    'search: "city, st" match',
    cityState.kind === 'match' && cityState.matches[0].id === 'ta-knox',
  );
  ...
  check(
    'search: no match -> none (no invented coords)',
    searchLocation(POOL, 'Miami, FL', names).kind === 'none',
  );
  check('search: empty -> none', searchLocation(POOL, '   ', names).kind === 'none');
  if (city.kind === 'match') {
    check(
      'search: origin centered in bounds',
      boundsContain(city.bounds, { lat: city.origin.lat, lng: city.origin.lng }),
    );
  }
}
...
/* ------------------------- clustering + bounds ------------------------- */
{
  ...
  check(
    'cluster: zoom grid monotonic',
    gridSizeForZoom(4) < gridSizeForZoom(8) && gridSizeForZoom(8) < gridSizeForZoom(13),
  );
  check(
    'bounds: contain all markers',
    markers.every((m) => boundsContain(bounds, m)),
  );
}
...
/* ------------------------- directions ------------------------- */
{
  ...
  check(
    'directions: standard url',
    ok === 'https://www.google.com/maps/dir/?api=1&destination=34.2727,-84.8077',
  );
  check(
    'directions: missing coords -> null',
    directionsUrl({ lat: undefined, lng: undefined }) === null,
  );
