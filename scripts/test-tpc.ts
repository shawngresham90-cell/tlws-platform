...
/* ------------------------- URL validation ------------------------- */
{
  ...
  check(
    'url: apex domain ok',
    ok.ok && ok.normalized === 'https://truckparkingclub.com/listing/abc',
  );
  const www = validateTpcUrl('https://www.truckparkingclub.com/listing/abc/');
  check(
    'url: www ok + trailing slash normalized',
    www.ok && www.normalized === 'https://www.truckparkingclub.com/listing/abc',
  );
  ...
  check(
    'url: deceptive subdomain rejected',
    !validateTpcUrl('https://truckparkingclub.com.evil.io/x').ok,
  );
  ...
  check(
    ...
    !validateTpcUrl('https://user:pass@truckparkingclub.com/x').ok,
  );
  ...
  check(
    'url: equality via normalization',
    tpcUrlsEqual('https://truckparkingclub.com/a/', 'https://truckparkingclub.com/a'),
  );
}
...
{
  check(
    'candidate: TPC-branded name',
    isTpcCandidate(ref({ name: 'Truck Parking Club - 11775 Snyder Rd' })),
  );
  check('candidate: parking category', isTpcCandidate(ref({ category: 'parking' })));
  check(
    'candidate: not when URL present',
    !isTpcCandidate(ref({ category: 'parking', tpcUrl: 'https://truckparkingclub.com/x' })),
  );
  check(
    'candidate: unrelated category not a candidate',
    !isTpcCandidate(ref({ category: 'cat-scales' })),
  );
  ...
  const rows = [
    ...
    ref({
      ...
      name: 'Lot B',
      category: 'parking',
      tpcUrl: 'https://truckparkingclub.com/l/2/',
    }),
    ref({
      id: 'c',
      name: 'Lot C',
    ...
    ref({
      ...
      name: 'Scale D',
      category: 'cat-scales',
      tpcUrl: 'https://truckparkingclub.com/l/3',
    }),
  ...
  check(
    'warn: duplicate URL across listings (normalized)',
    kinds('b').includes('duplicate-url') && kinds('c').includes('duplicate-url'),
  );
...
/* ------------------------- CSV pipeline ------------------------- */
{
  ...
  const live = new Map<string, TpcListingRef>([
    [
      A,
      ref({ id: A, name: 'Lot A', city: 'Dalton', state: 'GA', address: '1 Road', tpcUrl: null }),
    ],
    [
      B,
      ref({
        ...
        state: 'GA',
        address: '2 Road',
        tpcUrl: 'https://truckparkingclub.com/old',
      }),
    ],
  ]);
  ...
  const csv = toCsv([
    ...
    [
      ...
      'GA',
      '',
      'https://truckparkingclub.com/new-a',
      'set',
    ],
    [
      B,
      ...
      'Calhoun',
      'GA',
      'https://truckparkingclub.com/old',
      'https://truckparkingclub.com/new-b',
      'set',
    ],
    ...
    [
      ...
      'GA',
      '',
      'https://truckparkingclub.com/dup',
      'set',
    ],
  ]);
  ...
  check(
    'csv: duplicate id flagged on both',
    validated.filter((r) => r.problems.includes('duplicate-listing-id')).length === 2,
  );
  check(
    'csv: unknown id flagged',
    validated.find((r) => r.listing_id === C)?.problems.includes('unknown-listing-id') === true,
  );
  ...
  check(
    'csv: identity mismatch (wrong city)',
    mismatch.problems.includes('identity-mismatch') && !mismatch.applicable,
  );
  ...
  check(
    'csv: invalid url not applicable',
    invalid.problems.includes('invalid-url') && !invalid.applicable,
  );
