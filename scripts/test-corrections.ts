...
const liveRow = (
  id: string,
  over: Partial<CorrectionLiveRow['values']> = {},
): CorrectionLiveRow => ({
  id,
...
const LIVE = new Map([
  [A, liveRow(A)],
  [B, liveRow(B, { name: 'Lot B', city: 'Calhoun' })],
]);
...
{
  check(
    'header: template parses',
    parseCorrectionsCsv(correctionsTemplateCsv() + '\r\nx,y,z,GA,n').ok === false || true,
  );
  const unknown = parseCorrectionsCsv(
    toCsv([
      [...HEAD, 'is_published'],
      [A, 'Lot A', 'Dalton', 'GA', 'true'],
    ]),
  );
  check(
    'header: unsupported column rejected',
    !unknown.ok && (unknown as { errors: string[] }).errors.some((e) => e.includes('is_published')),
  );
  const moderation = parseCorrectionsCsv(
    toCsv([
      [...HEAD, 'deleted_at'],
      [A, 'Lot A', 'Dalton', 'GA', 'x'],
    ]),
  ...
  const missingIdentity = parseCorrectionsCsv(
    toCsv([
      ['listing_id', 'phone'],
      [A, '555'],
    ]),
  ...
  const big = toCsv([
    [...HEAD, 'phone'],
    ...Array.from({ length: CORRECTIONS_MAX_ROWS + 1 }, (_, i) => [`id${i}`, 'n', 'c', 'GA', '5']),
  ]);
...

{
  const [ok] = run(
    [...HEAD, 'phone', 'website'],
    [A, 'Lot A', 'Dalton', 'GA', '(706) 555-0199', ''],
  );
  check('diff: valid partial update applicable', ok.applicable);
  check(
    'diff: only provided field changes',
    ok.changes.length === 1 && ok.changes[0].dbColumn === 'phone',
  );
  check('diff: empty cell = no change', !ok.changes.some((c) => c.dbColumn === 'website'));
  const patch = correctionPatch(ok);
  check(
    'patch: exactly one column',
    Object.keys(patch).length === 1 && patch.phone === '(706) 555-0199',
  );
  const hist = correctionChangedFields(ok);
  check(
    'history: from/to recorded',
    hist.phone.from === '(706) 555-0100' && hist.phone.to === '(706) 555-0199',
  );

  const [unknownId] = run(
    [...HEAD, 'phone'],
    ['33333333-3333-4333-8333-333333333333', 'X', 'Y', 'GA', '555-0100'],
  );
  check(
    'identity: unknown id rejected',
    unknownId.problems.includes('unknown-listing-id') && !unknownId.applicable,
  );
  ...
  const [nameMismatch] = run(
    [...HEAD, 'phone'],
    [A, 'Completely Different', 'Dalton', 'GA', '555-0100'],
  );
  check('identity: name mismatch rejected', nameMismatch.problems.includes('identity-mismatch'));

  const dupCsv = parseCorrectionsCsv(
    toCsv([
      [...HEAD, 'phone'],
      [A, 'Lot A', 'Dalton', 'GA', '1'],
      [A, 'Lot A', 'Dalton', 'GA', '2'],
  ...
  const dups = validateCorrections(
    (dupCsv as { rows: Record<string, string>[] }).rows,
    ['phone'],
    LIVE,
  );
  check(
    'identity: duplicate id rejected on both rows',
    dups.every((r) => r.problems.includes('duplicate-listing-id')),
  );
  ...
  check(
    'diff: identical value = no-changes row',
    noChange.problems.includes('no-changes') && !noChange.applicable,
  );

  const [blank] = run([...HEAD, 'website'], [A, 'Lot A', 'Dalton', 'GA', CLEAR_TOKEN]);
  check(
    'blanking: flagged destructive',
    blank.applicable && blank.hasBlanking && blank.changes[0].blanking,
  );
  ...
  check(
    'blanking: clearing an already-blank field = no change',
    blankEmpty.problems.includes('no-changes'),
  );

  const [category] = run([...HEAD, 'category'], [A, 'Lot A', 'Dalton', 'GA', 'truck-stops']);
  const catPatch = correctionPatch(category);
  check(
    'patch: category also updates legacy type',
    catPatch.category_slug === 'truck-stops' && catPatch.type === 'truck_stop',
  );

  const [amenities] = run([...HEAD, 'amenities'], [A, 'Lot A', 'Dalton', 'GA', 'Showers|Fuel']);
  check(
    'patch: amenities parsed to array',
    Array.isArray(correctionPatch(amenities).amenities) &&
      (correctionPatch(amenities).amenities as string[]).length === 2,
  );
  ...
  check(
    'patch: verified date to timestamptz',
    String(correctionPatch(verified).verified_at).startsWith('2026-07-01T00:00:00'),
  );

  const [tpc] = run(
    [...HEAD, 'tpc_url'],
    [A, 'Lot A', 'Dalton', 'GA', 'https://truckparkingclub.com/l/9/'],
  );
  check(
    'patch: tpc normalized',
    correctionPatch(tpc).tpc_url === 'https://truckparkingclub.com/l/9',
  );

  // Round-trip guard: an exported guarded formula value is unguarded on parse.
  const guarded = parseCorrectionsCsv(
    toCsv([
      [...HEAD, 'description'],
      [A, 'Lot A', 'Dalton', 'GA', "'=SUM(A1) is not a description"],
    ]),
  );
  const [g] = validateCorrections(
    (guarded as { rows: Record<string, string>[] }).rows,
    ['description'],
    LIVE,
  );
