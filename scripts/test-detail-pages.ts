...
/* ------------------------- slug generation ------------------------- */
{
  ...
  check(
    ...
    detailSlugBase("Love's #550", 'Valdosta', 'GA') === 'love-s-550-valdosta-ga',
  );
  check(
    'slug: whitespace + case',
    detailSlugBase('  TA  LAKE Park ', 'Lake Park', 'ga') === 'ta-lake-park-lake-park-ga',
  );
  check('slug: never empty', detailSlugBase('***', '!!', '@') === 'listing');
  check(
    'slug: capped length',
    detailSlugBase('x'.repeat(300), 'y'.repeat(300), 'GA').length <= 100,
  );
  check(
    'slug: no leading/trailing hyphen',
    /^[a-z0-9].*[a-z0-9]$/.test(detailSlugBase('—Big Foot—', 'Cartersville', 'GA')),
  );
}
...
/* ------------------------- collision handling ------------------------- */
{
  ...
  check(
    'collision: free base untouched',
    uniqueDetailSlug('loves-dalton-ga', taken) === 'loves-dalton-ga',
  );
  check(
    'collision: suffix increments',
    uniqueDetailSlug('pilot-dalton-ga', taken) === 'pilot-dalton-ga-3',
  );
  check(
    'collision: first suffix is -2',
    uniqueDetailSlug('ta-dalton-ga', new Set(['ta-dalton-ga'])) === 'ta-dalton-ga-2',
  );
}
...
/* ------------------------- route-param guard ------------------------- */
{
  ...
  check(
    'guard: rejects path tricks',
    !isValidDetailSlug('../etc/passwd') && !isValidDetailSlug('a/b'),
  );
  check(
    'guard: rejects empty + hyphen edges',
    !isValidDetailSlug('') && !isValidDetailSlug('-abc') && !isValidDetailSlug('abc-'),
  );
...
const POOL: DirectoryEntry[] = [
  ...
  entry({
    ...
    lat: 30.94,
    lng: -83.19,
    detailSlug: 'ta-lake-park-lake-park-ga',
  }),
  entry({
    id: 'exit-scale',
    name: 'CAT Scale Exit 333',
    category: 'cat-scales',
    city: 'Calhoun',
    interstate: 'I-75',
    exitNumber: '333',
...
/* ------------------------- nearby ranking ------------------------- */
{
  ...
  check(
    'nearby: excludes the current listing',
    !sections.some((s) => s.items.some((i) => i.entry.id === 'current')),
  );
  check(
    'nearby: distance beats distance (near first)',
    bySlug['truck-stops']?.items[0]?.entry.id === 'near-stop',
  );
  check(
    'nearby: distance attached when both have coords',
    bySlug['truck-stops']?.items[0]?.distanceMiles != null &&
      (bySlug['truck-stops'].items[0].distanceMiles ?? 99) < 3,
  );
  check(
    'nearby: >75mi coordinate pairs dropped',
    !bySlug['truck-stops']?.items.some((i) => i.entry.id === 'far-stop'),
  );
  check(
    'nearby: exit window includes exit 333',
    bySlug['cat-scales']?.items.some((i) => i.entry.id === 'exit-scale'),
  );
  check(
    'nearby: exit window excludes exit 2 (326mi away)',
    !bySlug['cat-scales']?.items.some((i) => i.entry.id === 'far-exit-scale'),
  );
  check(
    'nearby: same-city fallback (no coords/exits)',
    bySlug['truck-washes']?.items.some((i) => i.entry.id === 'city-wash'),
  );
  check(
    'nearby: unrelated town excluded',
    !bySlug['truck-washes']?.items.some((i) => i.entry.id === 'other-town-wash'),
  );
  check(
    'nearby: sections capped',
    sections.every((s) => s.items.length <= NEARBY_SECTION_CAP),
    sections.map((s) => `${s.slug}:${s.items.length}`),
  );
  check('nearby: parking capped at 4 of 5', bySlug['parking']?.items.length === NEARBY_SECTION_CAP);
  check(
    'nearby: empty sections dropped',
    !bySlug['tire-repair'] && !bySlug['roadside-service'] && !bySlug['hotels-truck-parking'],
  );
  check(
    'nearby: no duplicate cards',
    sections.every((s) => new Set(s.items.map((i) => i.entry.id)).size === s.items.length),
  );

  // A coordinate-less current listing falls back to exit/city relations.
  const noCoords = entry({
    id: 'nc',
    name: 'No Coords Stop',
    city: 'Dalton',
    interstate: 'I-75',
  ...
  check(
    'nearby fallback: exit window works without coords',
    fb['cat-scales']?.items.some((i) => i.entry.id === 'exit-scale'),
  );
  check(
    'nearby fallback: same-city works without coords',
    fb['truck-stops']?.items.some((i) => i.entry.id === 'near-stop'),
  );
  check(
    'nearby fallback: no distances invented',
    fallback.every((s) => s.items.every((i) => i.distanceMiles == null)),
  );
}
...
/* ------------------------- meta copy ------------------------- */
{
  ...
  check(
    'meta: title has name + city',
    detailTitle(a).includes('Pilot #4558') && detailTitle(a).includes('Dalton, GA'),
  );
  check('meta: unique descriptions', detailDescription(a) !== detailDescription(b));
  check(
    'meta: description mentions exit + spaces',
    detailDescription(a).includes('Exit 328') && detailDescription(a).includes('80 truck spaces'),
  );
  check(
    'meta: description bounded',
    detailDescription(
      entry({ name: 'N'.repeat(120), city: 'C'.repeat(60), amenities: ['A'.repeat(60)] }),
    ).length <= 170,
  );
}
...
/* ------------------------- directions ------------------------- */
{
  ...
  check(
    'directions: address fallback (encoded, no invented coords)',
    byAddress ===
      `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent('324 Carbondale Rd, Dalton, GA, 30721')}`,
  );
  check('directions: nothing safe -> null', detailDirectionsUrl(entry({})) === null);
  check(
    'directions: 0/0 coords fall back to address',
    detailDirectionsUrl(entry({ lat: 0, lng: 0, address: 'x' }))?.includes('destination=x') ===
      true,
  );
}
...
/* ------------------------- detail schema ------------------------- */
{
  ...
  const reviews = [
    {
      ...
      title: 'Great',
      body: 'Clean showers',
      reviewerName: 'Driver A',
      createdAt: '2026-07-03T00:00:00Z',
    },
  ];
  const schema = listingDetailSchema(
    full,
    '/directory/location/pilot-4558-dalton-ga',
    { count: 3, average: 4.7 },
    reviews,
  ...
  check(
    'schema: canonical @id + mainEntityOfPage',
    String(schema['@id']).endsWith('/directory/location/pilot-4558-dalton-ga#listing') &&
      String(schema.mainEntityOfPage).endsWith('/directory/location/pilot-4558-dalton-ga'),
  );
  check('schema: geo present with coords', JSON.stringify(schema).includes('GeoCoordinates'));
  check(
    'schema: aggregateRating from approved stats',
    JSON.stringify(schema).includes('"reviewCount":3'),
  );
  check(
    'schema: review objects present',
    Array.isArray(schema.review) && (schema.review as unknown[]).length === 1,
  );
  ...
  const weighSchema = listingDetailSchema(weigh, '/directory/location/x', null, []) as Record<
    string,
    unknown
  >;
  check('schema: Place for weigh stations', weighSchema['@type'] === 'Place');
  check(
    'schema: no rating without approved reviews',
    !JSON.stringify(weighSchema).includes('aggregateRating'),
  );
  ...
  const noUpdated = listingDetailSchema(
    entry({ id: 'n1' }),
