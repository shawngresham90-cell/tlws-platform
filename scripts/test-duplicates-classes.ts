...
{
  check(
    'brand: store number stripped',
    brandKey('Pilot Travel Center #4558') === brandKey('Pilot Travel Center #192'),
  );
  check(
    'brand: different brands differ',
    brandKey("Love's Travel Stop #1") !== brandKey('Pilot Travel Center #1'),
  );
  check('similarity: identical = 1', nameSimilarity('TA Dalton', 'TA Dalton') === 1);
  check(
    'similarity: contained name high',
    nameSimilarity('CAT Scale Petro Atlanta', 'Petro Atlanta') >= 0.6,
  );
...
/* ------------------------- classification ------------------------- */
{
  ...
  check(
    'class: reasons name+address',
    exact.reasons.some((r) => r.includes('identical name')) &&
      exact.reasons.some((r) => r.includes('identical address')),
  );
  ...
  check(
    'class: probable duplicate (similar name, same address, same category)',
    probable.class === 'probable-duplicate',
    probable.class,
  );
  ...
  const subService = classifyPair(
    L({
      id: 'a',
      name: 'Petro Stopping Center Atlanta',
      category: 'truck-stops',
      address: '200 Hwy 41',
    }),
    L({
      id: 'b',
      name: 'CAT Scale — Petro Atlanta',
      category: 'cat-scales',
      address: '200 Hwy 41',
    }),
  );
  check(
    'class: shared-address sub-service',
    subService.class === 'shared-address-sub-service',
    subService.class,
  );
  ...
  check(
    'class: same coords, different category',
    coords.class === 'same-coords-diff-category',
    coords.class,
  );
  ...
  check(
    'class: same brand different exits is NOT a duplicate',
    brand.class === 'brand-multi-exit',
    brand.class,
  );
  ...
  check(
    'class: similar name different address',
    ['similar-name-diff-address', 'brand-multi-exit'].includes(similar.class),
    similar.class,
  );

  check(
    'class: deterministic',
    classifyPair(L({ id: 'a', name: 'X' }), L({ id: 'b', name: 'X' })).score ===
      classifyPair(L({ id: 'a', name: 'X' }), L({ id: 'b', name: 'X' })).score,
  );
}
...
/* ------------------------- pair finding + exclusion ------------------------- */
{
  ...
  check(
    'find: exact pair surfaced',
    pairs.some((p) => `${p.aId}|${p.bId}` === key('a', 'b')),
  );
  check(
    'find: shared phone surfaced',
    pairs.some((p) => `${p.aId}|${p.bId}` === key('d', 'e')),
    pairs.map((p) => `${p.aId}|${p.bId}`),
  );
  check('find: unrelated listing not paired', !pairs.some((p) => p.aId === 'c' || p.bId === 'c'));
  check(
    'find: sorted by score desc',
    pairs.every((p, i) => i === 0 || pairs[i - 1].score >= p.score),
  );

  const excluded = findClassifiedPairs(rows, new Set([key('a', 'b')]));
  check(
    'find: decided pair excluded',
    !excluded.some((p) => `${p.aId}|${p.bId}` === key('a', 'b')),
  );

  const csv = classifiedPairsCsv(pairs, new Map(rows.map((r) => [r.id, r])));
  check(
    'csv: export has header + rows',
    csv.startsWith('a_id,a_name') && csv.split('\r\n').length === pairs.length + 1,
  );
}
