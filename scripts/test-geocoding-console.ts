...
const LIVE = new Map<string, LiveListingRef>([
  [
    A,
    { id: A, name: 'Lot A', address: '1 Road', city: 'Dalton', state: 'GA', lat: null, lng: null },
  ],
  [
    B,
    {
...
/* ------------------------- backward compatibility ------------------------- */
{
  ...
  check(
    'compat: 15-column file still parses',
    parsed.rows.length === 1 && parsed.errors.length === 0,
  );
  check(
    'compat: empty evidence attached',
    parsed.rows[0].evidence.sourceUrls.length === 0 && parsed.rows[0].evidence.status === '',
  );
}
...
/* ------------------------- evidence columns ------------------------- */
{
  const csv = toCsv([
    ...
    [
      ...base15(A, 'Dalton', '1 Road', 'medium', 'manual-review'),
    ...
    [
      ...base15(B, 'Calhoun', '2 Road', 'high', 'ready'),
      '',
      '',
      '',
      '',
  ...
  check(
    'evidence: urls split + validated',
    e.sourceUrls.length === 2 && e.sourceUrls[1] === 'https://b.example',
  );
  check(
    'evidence: yes/no parsing',
    e.sideOfRoadConfirmed === true &&
      e.propertyConfirmed === false &&
      e.cityStateValidated === true,
  );
  check(
    'evidence: priority + concern + status',
    e.priority === 'high' && e.concernFlag && e.status === 'manual-review',
  );
  ...
  check(
    ...
    summary.byConfidence.medium === 1 && summary.byConfidence.high === 1,
  );
  check('summary: overwrite counted (B has live coords)', summary.overwrites === 1);
  check(
    'summary: status buckets',
    summary.byStatus['manual-review'] === 1 && summary.byStatus.ready === 1,
  );

  const staging = stagingCsv(validated, new Set([B]));
  check(
    'staging: only selected row, 15-column contract',
    staging.split('\r\n').length === 2 && staging.includes(B) && !staging.includes(A),
  );
  const reparsed = parseGeocodingCsv(staging);
  check(
    'staging: round-trips through the real parser',
    reparsed.rows.length === 1 && reparsed.errors.length === 0,
  );

  const queue = reviewQueueCsv(validated);
  check(
    'queue: only non-applicable rows',
    queue.includes(A) &&
      !queue
        .split('\r\n')
        .slice(1)
  ...
  check(
    'queue: evidence columns present',
...
/* ------------------------- priority sorting + osm ------------------------- */
{
  ...
  check(
    'osm: link shape',
    osmUrl(34.7, -84.9) === 'https://www.openstreetmap.org/?mlat=34.7&mlon=-84.9#map=17/34.7/-84.9',
  );
}
