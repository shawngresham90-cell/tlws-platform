...
# I-65 Alabama corridor, TN line (north) -> Gulf Coast / Mobile (south)
south_north = ["Ardmore","Elkmont","Athens","Tanner","Decatur","Priceville","Hartselle","Falkville",
    "Cullman","Good Hope","Hanceville","Warrior","Kimberly","Morris","Fultondale","Gardendale",
    "Birmingham","Homewood","Vestavia Hills","Hoover","Pelham","Alabaster","Calera","Jemison",
    "Thorsby","Clanton","Verbena","Prattville","Millbrook","Montgomery","Hope Hull","Greenville",
    "Georgiana","McKenzie","Evergreen","Castleberry","Brewton","Atmore","Bay Minette","Creola",
    "Saraland","Satsuma","Mobile","Theodore","Tillmans Corner"]
...
# ---- sources.md ----
o = [f"# Batch 8 — I-65 Alabama: Source Report\n",
     ...
     "confirmation, across corridor segments from the Tennessee line through Mobile and the Gulf Coast / I-10. Where official",
     "sites (Pilot/Flying J, Love's, TA/Petro, CAT Scale, Blue Beacon, TruckParkingClub, ALDOT) blocked",
...
r = [f"# Batch 8 — I-65 Alabama: Review Summary\n",
...
r.append("## Corridor coverage (TN line → Mobile / Gulf Coast)\n")
...
r.append(f"- Weigh/inspection stations included: **{len(ws)}** ({sum(1 for l in ws if l['published']=='yes')} published / {sum(1 for l in ws if l['published']=='no')} held). Highway coops without a civic street address + official source are held pending ALDOT confirmation.")
...
if dup:
    r.append(f"- Duplicate detection (`classifyPair`): vs Georgia **{dup.get('gaHits',0)}**, Tennessee **{dup.get('tnHits',0)}**, Kentucky **{dup.get('kyHits',0)}**, Ohio **{dup.get('ohHits',0)}**, Michigan **{dup.get('miHits',0)}**, Florida **{dup.get('flHits',0)}**, Indiana **{dup.get('inHits',0)}**, live DB **{dup.get('liveHits',0)}** matches; in-file co-location pairs: **{dup.get('inFile',0)}**; in-batch slug duplicates: **{dup.get('inBatchSlugDupes',0)}**.")
...
r.append("- Some brand/official sites (Pilot/Flying J, Love's, TA/Petro, CAT Scale, TruckParkingClub, ALDOT) rate-limit or block direct fetches; those facts were corroborated via 2+ independent directories and the affected fields left blank when unconfirmed.")
r.append("- Weigh-station direction/mile-marker detail is corroborated via aggregated truck directories; held until an official ALDOT source with a civic address confirms.")
...
# ---- validation.md ----
v = [f"# Batch 8 — I-65 Alabama: Validation Report\n",
...
if dup:
    v.append(f"- vs GA {dup.get('gaHits',0)} / TN {dup.get('tnHits',0)} / KY {dup.get('kyHits',0)} / OH {dup.get('ohHits',0)} / MI {dup.get('miHits',0)} / FL {dup.get('flHits',0)} / IN {dup.get('inHits',0)} / live {dup.get('liveHits',0)}; in-file co-location {dup.get('inFile',0)}; in-batch slug duplicates {dup.get('inBatchSlugDupes',0)}.")
