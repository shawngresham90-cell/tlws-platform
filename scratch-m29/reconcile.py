...
STATE_MERGES = [
    # Petro Dodge City co-located services catalogued by BOTH the Decatur/Cullman
    # (seg2) and Warrior/Gardendale (seg3) agents at the exit 299 segment boundary;
    # identical facility (426 AL-69 S, Hanceville, Exit 299). Keep seg2's spelling.
    ("CAT Scale - Petro Dodge City, Hanceville", "CAT Scale at Petro Dodge City"),
    ("TA Truck Service - Petro Dodge City", "TA Truck Service at Petro Dodge City"),
    # Conecuh County rest areas catalogued by BOTH the Montgomery/Atmore (seg6) and
    # Mobile/Saraland (seg7) agents at the Evergreen segment boundary; same NB and SB
    # facilities. Keep seg6's "I-65 …" spelling; seg7 back-fills the mile markers.
    ("Conecuh County Rest Area (Northbound)", "I-65 Conecuh County Rest Area (Northbound)"),
    ("Conecuh County Rest Area (Southbound)", "I-65 Conecuh County Rest Area (Southbound)"),
    # Cullman County rest area catalogued by both seg2 (MM 301, city=Cullman) and
    # seg3 (MM 300, blank city) at the exit 299/304 boundary — one bidirectional
    # facility near MM 300-301. Keep the seg2 row that carries the city; both held.
    ("Cullman County Rest Area (I-65 MM 300)", "Cullman County Rest Area (I-65 MM 301)"),
]
...
def _is_official(src):
    s = (src or "").lower()
    return any(d in s for d in ("alabama.gov", "aldot", "alea", ".gov", "dot.state.al.us"))
for l in rows:
    ...
    if not (has_addr and has_official):
        ...
        if not (l.get("hold_reason") or "").strip():
            l["hold_reason"] = ("Weigh/inspection station held pending an official ALDOT/ALEA source with a "
...
# official ALDOT/state source confirm the facility and its legal overnight status.
...
for l in rows:
    ...
    if not (has_addr and has_official):
        l["published"] = "no"
        if not (l.get("hold_reason") or "").strip():
            l["hold_reason"] = ("Public rest area / welcome center held pending an official ALDOT source "
