    ...
    <div className={`grid gap-5 ${cols}`}>
      {features.map((f) => (
        <Link
          key={f.title}
          href={f.href}
          className="placard lift group flex flex-col p-4 transition-colors hover:border-signal focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal focus-visible:ring-offset-2 focus-visible:ring-offset-asphalt sm:p-6"
        >
          <h3 className="font-display text-xl uppercase text-signal">{f.title}</h3>
          <p className="mt-2 flex-1 text-base text-muted">{f.description}</p>
