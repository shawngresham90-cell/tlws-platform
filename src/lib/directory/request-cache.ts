 ...
 * the exit-entries read 2× per regeneration. The listing detail page has the
 * same shape: metadata and body both resolve the same slug.
 *
 * React's `cache()` collapses those into one read per REQUEST. It is
 * deliberately request-scoped, not a TTL cache: ISR (`revalidate = 300`) plus
 * the admin's `revalidatePath()` purges stay the sole cross-request freshness
 * mechanism, so a newly published location appears on exactly the same
 * schedule as before.
 *
...
export const cachedEntryByDetailSlugResult = cache(getEntryByDetailSlugResult);
