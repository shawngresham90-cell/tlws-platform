...
export const listingSchema = z.object({
  ...
  exit_number: optional(z.string().trim().max(20)),
...
export function parseListingForm(
  ...
  const raw = {
    ...
    interstate: formData.get('interstate') ?? '',
    exit_number: formData.get('exit_number') ?? '',
...
export function toRow(input: ListingInput) {
  return {
    ...
    interstate: input.interstate ?? null,
    exit_number: input.exit_number ?? null,
