...
def _is_official(src):
    s = (src or "").lower()
    return any(d in s for d in ("tn.gov", "tdot", "tennessee.gov", ".gov", "tdot.tn.gov", "thp"))
for l in rows:
    nm = (l.get("name") or "")
    ...
    if not (has_addr and has_official):
        ...
        if not (l.get("hold_reason") or "").strip():
            l["hold_reason"] = ("Weigh station / rest area / welcome center held pending an official "
                                "TDOT/THP source with a civic street address confirming direction, "
...
if _live_path.exists():
    ...
    print("WARN: scratch-b11/live.json missing — existing-TN exclusion pass skipped", file=sys.stderr)
