...
# I-65 Tennessee corridor, Alabama line (south) -> Kentucky line (north)
south_north = ["Ardmore","Elkton","Pulaski","Lynnville","Cornersville","Columbia","Spring Hill",
    "Thompson's Station","Franklin","Brentwood","Nashville","Madison","Goodlettsville","Millersville",
    "White House","Cottontown","Portland","Mitchellville"]
...
# ---- sources.md ----
o = [f"# Batch 9 — I-65 Tennessee: Source Report\n",
     ...
     "confirmation, across corridor segments from the Alabama line through Nashville to the Kentucky line. Where official",
     "sites (Pilot/Flying J, Love's, TA/Petro, CAT Scale, Blue Beacon, TruckParkingClub, TDOT) blocked",
...
r = [f"# Batch 9 — I-65 Tennessee: Review Summary\n",
...
r.append("## Corridor coverage (Alabama line → Nashville → Kentucky line)\n")
...
r.append(f"- Weigh/inspection stations included: **{len(ws)}** ({sum(1 for l in ws if l['published']=='yes')} published / {sum(1 for l in ws if l['published']=='no')} held). Highway coops without a civic street address + official source are held pending TDOT/THP confirmation.")
...
if dup:
    r.append(f"- Duplicate detection (`classifyPair`): vs Georgia **{dup.get('gaHits',0)}**, Tennessee **{dup.get('tnHits',0)}**, Kentucky **{dup.get('kyHits',0)}**, Ohio **{dup.get('ohHits',0)}**, Michigan **{dup.get('miHits',0)}**, Florida **{dup.get('flHits',0)}**, Indiana **{dup.get('inHits',0)}**, Alabama **{dup.get('alHits',0)}**, live DB **{dup.get('liveHits',0)}** matches; in-file co-location pairs: **{dup.get('inFile',0)}**; in-batch slug duplicates: **{dup.get('inBatchSlugDupes',0)}**.")
...
r.append("- Weigh-station direction/mile-marker detail is corroborated via aggregated truck directories; held until an official TDOT/THP source with a civic address confirms.")
...
# ---- validation.md ----
v = [f"# Batch 9 — I-65 Tennessee: Validation Report\n",
...
if dup:
    v.append(f"- vs GA {dup.get('gaHits',0)} / TN {dup.get('tnHits',0)} / KY {dup.get('kyHits',0)} / OH {dup.get('ohHits',0)} / MI {dup.get('miHits',0)} / FL {dup.get('flHits',0)} / IN {dup.get('inHits',0)} / AL {dup.get('alHits',0)} / live {dup.get('liveHits',0)}; in-file co-location {dup.get('inFile',0)}; in-batch slug duplicates {dup.get('inBatchSlugDupes',0)}.")
