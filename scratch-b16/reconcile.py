#!/usr/bin/env python3
"""Batch 16 reconcile (I-40 North Carolina West): merge seg*.json (agent output) into the
...
def _is_official(src):
    s = (src or "").lower()
    return any(d in s for d in ("ncdot.gov", "nc.gov", ".gov", "ncshp", "visitnc"))
for l in rows:
    nm = (l.get("name") or "")
    ...
    if not (has_addr and has_official):
        ...
        if not (l.get("hold_reason") or "").strip():
            l["hold_reason"] = ("Weigh station / rest area / welcome center held pending an official "
                                "NCDOT/NCSHP source with a civic street address confirming direction, "
