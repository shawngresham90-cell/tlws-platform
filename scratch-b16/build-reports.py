#!/usr/bin/env python3
"""Batch 16 report builder (I-40 North Carolina West):
...
REPO = HERE.parent
STEM = "i40-north-carolina-west-batch-016"
DATE = "2026-07-13"
STATES = ("NC",)
INTERSTATE = "I-40"
...
west_east = ["Waynesville","Lake Junaluska","Canton","Candler","Asheville","Swannanoa","Black Mountain",
             "Ridgecrest","Old Fort","Marion","Nebo","Glen Alpine","Morganton","Valdese","Hildebran",
             "Hickory","Conover","Newton","Claremont"]
...
# ---- sources.md ----
o = [f"# Batch 16 — I-40 North Carolina West: Source Report\n",
     ...
     "confirmation, across four corridor segments covering the western half of I-40 in North",
     "Carolina: the Tennessee line and Pigeon River Gorge through Waynesville, Asheville,",
     "Old Fort Mountain, Marion and Morganton to Hickory/Conover (exits 0-132).",
     "Where official sites (Pilot/Flying J, Love's, TA/Petro, CAT Scale, TruckParkingClub,",
     "NCDOT/ncshp.gov) blocked direct fetches, facts were captured via search snippets and corroborated by 2+ directories.\n",
...
r = [f"# Batch 16 — I-40 North Carolina West: Review Summary\n",
...
r.append("## Corridor coverage (TN line/gorge → Waynesville → Asheville → Old Fort Mtn → Marion → Morganton → Hickory)\n")
...
r.append(f"- Weigh/inspection stations included: **{len(ws)}** ({sum(1 for l in ws if l['published']=='yes')} published / {sum(1 for l in ws if l['published']=='no')} held). Stations without a civic street address + official source are held pending NCDOT/NCSHP confirmation.")
...
nc_live = sum(1 for l in live_rows if (l.get('state') or '')=='NC')
r.append(f"- North Carolina is a NEW state for the directory: production has **{nc_live}** existing NC listings, so no existing-state collision is possible. All candidates were still cross-checked (`classifyPair`) against every merged batch CSV, all six pending unmerged batches (b010-b015) and the full live DB: zero hits of any score. In-file pairs are the legitimate TA Candler host + CAT Scale + TA Truck Service co-locations.")
...
# ---- validation.md ----
v = [f"# Batch 16 — I-40 North Carolina West: Validation Report\n",
