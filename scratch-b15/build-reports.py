#!/usr/bin/env python3
"""Batch 15 report builder (I-40 Arkansas):
...
west_east = ["Van Buren","Alma","Mulberry","Ozark","Clarksville","London","Russellville","Atkins",
             "Morrilton","Conway","Mayflower","North Little Rock","Lonoke","Carlisle","Hazen",
             "Brinkley","Wheatley","Palestine","Forrest City","Widener","Heth","Earle","West Memphis"]
...
# ---- sources.md ----
o = [f"# Batch 15 — I-40 Arkansas: Source Report\n",
     ...
     "confirmation, across six corridor segments spanning the full 284-mile I-40 Arkansas",
     "crossing: the Oklahoma line at Fort Smith/Van Buren through Russellville, Conway,",
     "North Little Rock and Brinkley to the Mississippi River at West Memphis.",
     "Where official sites (Pilot/Flying J, Love's, TA/Petro, CAT Scale, TruckParkingClub,",
     "ARDOT/arkansas.gov) blocked direct fetches, facts were captured via search snippets and corroborated by 2+ directories.\n",
...
r = [f"# Batch 15 — I-40 Arkansas: Review Summary\n",
...
r.append("## Corridor coverage (OK line/Van Buren → Russellville → Conway → Little Rock → Brinkley → West Memphis)\n")
...
r.append(f"- Weigh/inspection stations included: **{len(ws)}** ({sum(1 for l in ws if l['published']=='yes')} published / {sum(1 for l in ws if l['published']=='no')} held). Stations without a civic street address + official source are held pending ARDOT/AHP confirmation.")
...
ar_live = sum(1 for l in live_rows if (l.get('state') or '')=='AR')
r.append(f"- Arkansas is a NEW state for the directory: production has **{ar_live}** existing AR listings, so no existing-state collision is possible. All candidates were still cross-checked (`classifyPair`) against every merged batch CSV, all five pending unmerged batches (b010-b014) and the full live DB; only shared-corporate-contact chain false positives surfaced (same Speedco/Love's corporate phone+website or same Road Ranger chain name at a different city, state and street address). Memphis TN facilities across the river were excluded (covered by the I-40 TN batch).")
...
r.append("- ardot.gov / arkansas.gov / arkansas.com 403 through the research proxy, so the officially-confirmed Alma and Lehi/Riverside (West Memphis) weigh stations, the ARDOT rest areas (MM 36/199/235/242.6) and the Van Buren Welcome Center are HELD pending official civic addresses rather than published on unofficial data. Store-number corrections were sourced during research (West Memphis Flying J is #607, the MLK Dr Pilot is #429; the 'TA West Memphis' is actually Petro #311).")
...
# ---- validation.md ----
v = [f"# Batch 15 — I-40 Arkansas: Validation Report\n",
