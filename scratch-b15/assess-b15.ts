/**
 * Batch 15 (I-40 Arkansas) assessment — read-only, drives the REAL production libs:
 *  prepareImport, assessExpansion, classifyPair, scoreCompleteness, normalizeText.
 *
 *   npx esbuild scratch-b15/assess-b15.ts --bundle --platform=node --format=cjs \
 *     --alias:@=./src --outfile=.next/assess-b15.cjs && node .next/assess-b15.cjs \
 *     data/imports/i40-arkansas-batch-015.csv scratch-b15/live.json scratch-b15
 */
...
if (!masterPath || !livePath || !outDir) { console.error('usage: assess-b15 <master.csv> <live.json> <out-dir>'); process.exit(2); }
...
writeFileSync(`${outDir}/../data/imports/i40-arkansas-batch-015-expansion-report.csv`, expansionReportCsv(report));
...
const newPairs = csvToPairs(masterPath, 'b15');
...
const ky65Hits = crossCheck('PENDING I-65 Kentucky b010', csvToPairs('scratch-b15/pending-i65-kentucky-batch-010.csv', 'ky65p'));
const tn24Hits = crossCheck('PENDING I-24 Tennessee b011', csvToPairs('scratch-b15/pending-i24-tennessee-batch-011.csv', 'tn24p'));
const ky24Hits = crossCheck('PENDING I-24 Kentucky b012', csvToPairs('scratch-b15/pending-i24-kentucky-batch-012.csv', 'ky24p'));
const ilga24Hits = crossCheck('PENDING I-24 IL+GA b013', csvToPairs('scratch-b15/pending-i24-illinois-georgia-batch-013.csv', 'ilga24p'));
const tn40Hits = crossCheck('PENDING I-40 TN b014', csvToPairs('scratch-b15/pending-i40-tennessee-batch-014.csv', 'tn40p'));
...
writeFileSync(`${outDir}/dup-summary.json`, JSON.stringify({ gaHits, tnHits, kyHits, ohHits, miHits, flHits, inHits, alHits, tn65Hits, ky65Hits, tn24Hits, ky24Hits, ilga24Hits, tn40Hits, liveHits, inFile, inBatchSlugDupes: inBatchSlugDupes.length }, null, 1));
console.log('DONE');
