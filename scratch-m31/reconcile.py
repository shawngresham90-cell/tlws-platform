...
def _is_official(src):
    s = (src or "").lower()
    return any(d in s for d in ("ky.gov", "kytc", "kentucky.gov", ".gov", "transportation.ky.gov", "ksp"))
for l in rows:
    ...
    if not (has_addr and has_official):
        ...
        if not (l.get("hold_reason") or "").strip():
            l["hold_reason"] = ("Weigh/inspection station held pending an official KYTC/KSP source with a "
...
# official KYTC/state source confirm the facility and its legal overnight status.
...
for l in rows:
    ...
    if not (has_addr and has_official):
        l["published"] = "no"
        if not (l.get("hold_reason") or "").strip():
            l["hold_reason"] = ("Public rest area / welcome center held pending an official KYTC source "
...
if _live_path.exists():
    ...
    print("WARN: scratch-m31/live.json missing — existing-TN exclusion pass skipped", file=sys.stderr)
