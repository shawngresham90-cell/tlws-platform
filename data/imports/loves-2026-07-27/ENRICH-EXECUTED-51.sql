-- Love's enrichment — EXECUTED SET (51 of the 62 staged rows), 2026-07-28.
--
-- Re-authored from ENRICH-EXISTING.sql for live conditions found in the
-- pre-write sweep. Guards are IDENTICAL plus one ADDED guard (published-pin
-- collision, self-excluded). Nothing was weakened. Excluded, with reasons, in
-- EXECUTION-QUARANTINE-2026-07-28.md:
--   Already carry coordinates (blank-only rule): #307 #325 #359 #550 #698
--     #735 #801 (GA), #364 #861 (TN) — filled earlier by the interpolation
--     enrichment line; the official-coordinate upgrade needs its own
--     authorization because it would overwrite a non-null value.
--   Published-pin collision inside the ±0.0015° box (ADDED guard): #317 VA
--     (Pilot Travel Center #4651), #451 IN (Pilot Travel Center #37).
...
begin
  ...
  if batch <> 51 then
    raise exception 'Expected 51 staged rows, found %.', batch;
  end if;
  ...
  -- 4. Coordinates plausible and in envelope.
  ...
  -- 6b. ADDED: the new pin must not land within ~150 m of any OTHER published pin.
  select count(*) into n from _enr e join public.locations l
    on l.deleted_at is null and l.is_published and l.id <> e.db_id and l.lat is not null
   and abs(l.lat - e.lat) < 0.0015 and abs(l.lng - e.lng) < 0.0015;
  if n <> 0 then raise exception '% enrichment coordinate(s) collide with a published pin.', n; end if;
  ...
  -- 8. Enrichment must never publish or feature.
  select count(*) into n from _enr e join public.locations l on l.id = e.db_id
   where l.is_featured;
