-- TA/Petro full publication — remaining 294 rows.
--
-- Publishes is_published false -> true for exactly the 294 verified batch rows
-- in manifest-294.json, in ~6 passes, each STATE in its own guarded transaction
-- (each do-block is one implicit transaction). Only is_published is written.
--
-- Per state: scoped by id AND source label AND is_published=false AND
-- deleted_at is null, with GET DIAGNOSTICS ROW_COUNT = expected; any mismatch
-- raises, aborting that state's block and rolling it back automatically. Blocks
-- are idempotent: after a state is published, its precondition matches 0 rows,
-- ROW_COUNT != expected, and the block raises instead of double-acting — so a
-- re-run of an already-applied state is a safe no-op abort, not a change.
--
-- Source label: official-ta-petro-20260725-5ebe0e9f (from CHECKPOINT.md). Exact IDs only, never name match.
-- is_indexable is never referenced. Held/excluded networks are not in the set.


-- ===== PASS 1: AL, AR, AZ, CA, CO, CT (46 rows) =====
...
begin
  ...
  if n <> 8 then
    raise exception 'AL PUBLISH GUARD: expected 8, matched %. State rolled back.', n;
  end if;
  raise notice 'AL: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 3 then
    raise exception 'AR PUBLISH GUARD: expected 3, matched %. State rolled back.', n;
  end if;
  raise notice 'AR: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 10 then
    raise exception 'AZ PUBLISH GUARD: expected 10, matched %. State rolled back.', n;
  end if;
  raise notice 'AZ: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 13 then
    raise exception 'CA PUBLISH GUARD: expected 13, matched %. State rolled back.', n;
  end if;
  raise notice 'CA: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 9 then
    raise exception 'CO PUBLISH GUARD: expected 9, matched %. State rolled back.', n;
  end if;
  raise notice 'CO: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 3 then
    raise exception 'CT PUBLISH GUARD: expected 3, matched %. State rolled back.', n;
  end if;
  raise notice 'CT: published % rows.', n;
end $$;


-- ===== PASS 2: FL, GA, IA, ID, IL, IN, KS (50 rows) =====
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 4 then
    raise exception 'FL PUBLISH GUARD: expected 4, matched %. State rolled back.', n;
  end if;
  raise notice 'FL: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 7 then
    raise exception 'GA PUBLISH GUARD: expected 7, matched %. State rolled back.', n;
  end if;
  raise notice 'GA: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 6 then
    raise exception 'IA PUBLISH GUARD: expected 6, matched %. State rolled back.', n;
  end if;
  raise notice 'IA: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 2 then
    raise exception 'ID PUBLISH GUARD: expected 2, matched %. State rolled back.', n;
  end if;
  raise notice 'ID: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 12 then
    raise exception 'IL PUBLISH GUARD: expected 12, matched %. State rolled back.', n;
  end if;
  raise notice 'IL: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 9 then
    raise exception 'IN PUBLISH GUARD: expected 9, matched %. State rolled back.', n;
  end if;
  raise notice 'IN: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 10 then
    raise exception 'KS PUBLISH GUARD: expected 10, matched %. State rolled back.', n;
  end if;
  raise notice 'KS: published % rows.', n;
end $$;


-- ===== PASS 3: KY, LA, MI, MN, MO, MS, MT, NC, ND, NE (52 rows) =====
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 3 then
    raise exception 'KY PUBLISH GUARD: expected 3, matched %. State rolled back.', n;
  end if;
  raise notice 'KY: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 12 then
    raise exception 'LA PUBLISH GUARD: expected 12, matched %. State rolled back.', n;
  end if;
  raise notice 'LA: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 4 then
    raise exception 'MI PUBLISH GUARD: expected 4, matched %. State rolled back.', n;
  end if;
  raise notice 'MI: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 4 then
    raise exception 'MN PUBLISH GUARD: expected 4, matched %. State rolled back.', n;
  end if;
  raise notice 'MN: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 12 then
    raise exception 'MO PUBLISH GUARD: expected 12, matched %. State rolled back.', n;
  end if;
  raise notice 'MO: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 4 then
    raise exception 'MS PUBLISH GUARD: expected 4, matched %. State rolled back.', n;
  end if;
  raise notice 'MS: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 2 then
    raise exception 'MT PUBLISH GUARD: expected 2, matched %. State rolled back.', n;
  end if;
  raise notice 'MT: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 3 then
    raise exception 'NC PUBLISH GUARD: expected 3, matched %. State rolled back.', n;
  end if;
  raise notice 'NC: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 5 then
    raise exception 'ND PUBLISH GUARD: expected 5, matched %. State rolled back.', n;
  end if;
  raise notice 'ND: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 3 then
    raise exception 'NE PUBLISH GUARD: expected 3, matched %. State rolled back.', n;
  end if;
  raise notice 'NE: published % rows.', n;
end $$;


-- ===== PASS 4: NH, NJ, NM, NV, NY, OH (43 rows) =====
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 1 then
    raise exception 'NH PUBLISH GUARD: expected 1, matched %. State rolled back.', n;
  end if;
  raise notice 'NH: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 4 then
    raise exception 'NJ PUBLISH GUARD: expected 4, matched %. State rolled back.', n;
  end if;
  raise notice 'NJ: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 8 then
    raise exception 'NM PUBLISH GUARD: expected 8, matched %. State rolled back.', n;
  end if;
  raise notice 'NM: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 10 then
    raise exception 'NV PUBLISH GUARD: expected 10, matched %. State rolled back.', n;
  end if;
  raise notice 'NV: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 7 then
    raise exception 'NY PUBLISH GUARD: expected 7, matched %. State rolled back.', n;
  end if;
  raise notice 'NY: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 13 then
    raise exception 'OH PUBLISH GUARD: expected 13, matched %. State rolled back.', n;
  end if;
  raise notice 'OH: published % rows.', n;
end $$;


-- ===== PASS 5: OK, OR, PA, RI, SC, SD, TN, UT, VA (45 rows) =====
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 7 then
    raise exception 'OK PUBLISH GUARD: expected 7, matched %. State rolled back.', n;
  end if;
  raise notice 'OK: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 6 then
    raise exception 'OR PUBLISH GUARD: expected 6, matched %. State rolled back.', n;
  end if;
  raise notice 'OR: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 13 then
    raise exception 'PA PUBLISH GUARD: expected 13, matched %. State rolled back.', n;
  end if;
  raise notice 'PA: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 1 then
    raise exception 'RI PUBLISH GUARD: expected 1, matched %. State rolled back.', n;
  end if;
  raise notice 'RI: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 5 then
    raise exception 'SC PUBLISH GUARD: expected 5, matched %. State rolled back.', n;
  end if;
  raise notice 'SC: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 3 then
    raise exception 'SD PUBLISH GUARD: expected 3, matched %. State rolled back.', n;
  end if;
  raise notice 'SD: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 2 then
    raise exception 'TN PUBLISH GUARD: expected 2, matched %. State rolled back.', n;
  end if;
  raise notice 'TN: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 3 then
    raise exception 'UT PUBLISH GUARD: expected 3, matched %. State rolled back.', n;
  end if;
  raise notice 'UT: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 5 then
    raise exception 'VA PUBLISH GUARD: expected 5, matched %. State rolled back.', n;
  end if;
  raise notice 'VA: published % rows.', n;
end $$;


-- ===== PASS 6: WA, WI, WV, WY, TX (58 rows) =====
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 4 then
    raise exception 'WA PUBLISH GUARD: expected 4, matched %. State rolled back.', n;
  end if;
  raise notice 'WA: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 7 then
    raise exception 'WI PUBLISH GUARD: expected 7, matched %. State rolled back.', n;
  end if;
  raise notice 'WI: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 4 then
    raise exception 'WV PUBLISH GUARD: expected 4, matched %. State rolled back.', n;
  end if;
  raise notice 'WV: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 4 then
    raise exception 'WY PUBLISH GUARD: expected 4, matched %. State rolled back.', n;
  end if;
  raise notice 'WY: published % rows.', n;
end $$;
...
begin
  ...
  get diagnostics n = row_count;
  if n <> 39 then
    raise exception 'TX PUBLISH GUARD: expected 39, matched %. State rolled back.', n;
  end if;
  raise notice 'TX: published % rows.', n;
end $$;
