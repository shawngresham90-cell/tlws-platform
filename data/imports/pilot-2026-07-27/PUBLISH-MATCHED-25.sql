...
-- equal the official values exactly, and the last read-only sweep found no
...
-- updated_at). Canary first — SIX rows, one per represented state, three
-- brand families — then one guarded transaction per state for the remaining
-- 19. Any guard failure aborts that transaction; the failing row is
-- quarantined, never forced.
--
-- Expected if all 25 pass: live rows stay 2,265 · published 1,862 -> 1,887 ·
-- with_coords stay 1,351 · published_unmappable stays 555 · Gate 3a stays
-- 809/820 · Gate 3b 763/803 -> 788/803.

-- ===== CANARY — 6 rows, one per state (#90 FL, #627 GA, #875 MD, #58 NC, #713 SC, #384 VA) (6 rows)
begin;
...
begin
  ...
  if batch <> 6 then raise exception 'Expected 6 staged row(s), found %.', batch; end if;
  ...
  if n <> 6 then raise exception 'Expected to publish exactly 6 row(s), published %.', n; end if;
...
-- CANARY AUDIT before continuing (read-only): all 6 published; each satisfies
-- the directory / trip-planner data contract (is_published AND lat/lng NOT
-- NULL AND parking_spaces > 0 AND category truck-stops => route-usable);
-- none featured / indexable / overnight; directory published 1,862 -> 1,868;
-- control digest unchanged. Only then run the per-state remainder:
...
begin
  select count(*) into batch from _pub;
  if batch <> 2 then raise exception 'Expected 2 staged row(s), found %.', batch; end if;
  ...
  update public.locations l
   ...
   where l.id = p.db_id and not l.is_published;
  get diagnostics n = row_count;
  if n <> 2 then raise exception 'Expected to publish exactly 2 row(s), published %.', n; end if;
