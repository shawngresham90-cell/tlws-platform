#!/usr/bin/env python3
"""Batch 13 reconcile (I-24 Illinois + Georgia corridor completion): merge seg*.json (agent output) into the
...
def _is_official(src):
    s = (src or "").lower()
    return any(d in s for d in ("illinois.gov", "idot", ".gov", "isp.state", "dot.ga.gov", "georgia.gov", "dps.georgia"))
for l in rows:
    nm = (l.get("name") or "")
    ...
    if not (has_addr and has_official):
        ...
        if not (l.get("hold_reason") or "").strip():
            l["hold_reason"] = ("Weigh station / rest area / welcome center held pending an official "
                                "IDOT/ISP or GDOT/DPS source with a civic street address confirming direction, "
