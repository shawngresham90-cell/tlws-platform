/**
 * Batch 13 (I-24 Illinois + Georgia) assessment — read-only, drives the REAL production libs:
 *  prepareImport, assessExpansion, classifyPair, scoreCompleteness, normalizeText.
 *
 *   npx esbuild scratch-b13/assess-b13.ts --bundle --platform=node --format=cjs \
 *     --alias:@=./src --outfile=.next/assess-b13.cjs && node .next/assess-b13.cjs \
 *     data/imports/i24-illinois-georgia-batch-013.csv scratch-b13/live.json scratch-b13
 */
...
if (!masterPath || !livePath || !outDir) { console.error('usage: assess-b13 <master.csv> <live.json> <out-dir>'); process.exit(2); }
...
writeFileSync(`${outDir}/../data/imports/i24-illinois-georgia-batch-013-expansion-report.csv`, expansionReportCsv(report));
...
const newPairs = csvToPairs(masterPath, 'b13');
...
const ky65Hits = crossCheck('PENDING I-65 Kentucky b010', csvToPairs('scratch-b13/pending-i65-kentucky-batch-010.csv', 'ky65p'));
const tn24Hits = crossCheck('PENDING I-24 Tennessee b011', csvToPairs('scratch-b13/pending-i24-tennessee-batch-011.csv', 'tn24p'));
const ky24Hits = crossCheck('PENDING I-24 Kentucky b012', csvToPairs('scratch-b13/pending-i24-kentucky-batch-012.csv', 'ky24p'));
...
writeFileSync(`${outDir}/dup-summary.json`, JSON.stringify({ gaHits, tnHits, kyHits, ohHits, miHits, flHits, inHits, alHits, tn65Hits, ky65Hits, tn24Hits, ky24Hits, liveHits, inFile, inBatchSlugDupes: inBatchSlugDupes.length }, null, 1));
console.log('DONE');
